...
class ExtensionSola extends Extension
{
    ...
    public function onControllerPagesSaleOrderTabs_UpdateData()
    {
        ...
        if (IS_ADMIN && $that->user->isLogged()) {
            if ($order_info['payment_method_key'] != 'sola') {
                return null;
            }
            ...
            $that->data['link_payment_details'] = $that->html->getSecureURL('sale/order/payment_details', '&order_id='.$order_id.'&extension=sola');
    ...
    public function onControllerPagesSaleOrder_UpdateData()
    {
        ...
        if (IS_ADMIN
            ...
            if($that->data['order_info']['payment_method_key'] != 'sola'){
                return;
            }

            if (has_value($payment_method_data['xResult']) && $payment_method_data['xResult'] == 'A') {
                $that->loadLanguage('sola/sola');
                ...
                $data = array();
                $data['text_payment_status'] = $that->language->get('sola_text_payment_status');
                if (
                ...
                $view->batchAssign($that->language->getASet('sola/sola'));
                $view->batchAssign($tpl_data);
                $that->view->addHookVar('extension_payment_details', $view->fetch('pages/sale/sola_payment_details.tpl'));
            }
        }
    }
    ...
    private function _get_capture_form($data = array(), $payment_method_data = array())
    {
        ...
        $data['text_payment_status'] = $that->language->get('sola_text_payment_status');
        $data['payment_status'] = $that->language->get('sola_text_pending_authorization');
        $data['sola_capture_amount'] = $that->html->buildInput(
            array(
                'name'  => 'sola_capture_amount',
                'value' => $authAmount,
                'style' => 'no-save',
                'attr'  => 'disabled',
            )
        );
        $data['text_capture_funds'] = $that->language->get('sola_text_capture_funds');
        $data['sola_capture_submit'] = $that->html->buildElement(array(
            'type' => 'button',
            'text' => $that->language->get('sola_text_capture'),
            'name' => 'sola_capture_submit',
        ));

        $data['sola_capture_action'] = $that->html->getSecureURL(
            'r/extension/sola/capture',
    ...
    private function _get_refund_form($data = array(), $payment_method_data = array(), $not_refunded = 0)
    {
        ...
        $data['payment_status'] = $that->language->get('sola_text_processing');
        if (!(float)$alreadyRefunded) {
            $data['sola_refund_amount'] = $that->html->buildInput(
                array(
                    'name'  => 'sola_refund_amount',
                    'value' => (float)$payment_method_data['xAuthAmount'],
                    'style' => 'no-save',
                )
            );
            $data['text_do_refund'] = $that->language->get('sola_text_do_refund');
            $data['sola_refund_submit'] = $that->html->buildElement(array(
                'type'  => 'button',
                'text'  => $that->language->get('sola_text_refund'),
                'title' => $that->language->get('sola_text_refund'),
                'name'  => 'sola_refund_submit',
            ));

            $params = '&order_id='.(int)$that->data['order_info']['order_id']
                        .'&currency='.$that->data['currency']['code'];

            $data['sola_refund_action'] = $that->html->getSecureURL(
                'r/extension/sola/refund',
                $params
            );

        } else {
            $data['payment_status'] = $that->language->get('sola_text_refunded');
        }
