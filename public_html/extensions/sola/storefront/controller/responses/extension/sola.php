...
class ControllerResponsesExtensionSola extends AController {
    protected $iFieldsVersion = '2.6.2006.0102';

    public function main()
    {
        $this->loadLanguage('sola/sola');
        $this->loadModel('checkout/order');
        $data['iFieldsVersion'] = $this->iFieldsVersion;

        $data['action'] = $this->html->getSecureURL('r/extension/sola/send');
        ...
        $data['customer_name'] = $form->getFieldHtml(
            [
                ...
                'placeholder' => $this->language->get('sola_text_your_name')
            ]
        );
        $data['routing_number'] = $form->getFieldHtml(
            [
                ...
                'name'  => 'xRouting',
                'value' => '',
                'placeholder' => $this->language->get('sola_text_routing_number')
            ]
        );
        ...
        $data['ebt_number'] = $form->getFieldHtml(
            [
                ...
                'name'  => 'EBTCardNum',
                'value' => '',
                'placeholder' => $this->language->get('sola_text_ebt_number')
            ]
        );

        $data['ebt_init_url'] = $this->html->getSecureURL('r/extension/sola/ebt_init','',true);
        $data['sola_text_ebt'] = $this->language->get('sola_text_ebt','sola/sola');
        ...
        $this->processTemplate('responses/sola.tpl');
    }
    ...
    public function send($redirect = false)
    {
        ...
        $data['xKey'] = $this->config->get('sola_transaction_key');
        $data['xVersion'] = '4.5.8';
        ...
        if($this->request->post['method'] == 'cc') {
            $data['xCommand'] = ($this->config->get('sola_transaction_method') == 'capture') ? 'cc:sale' : 'cc:authonly';
        ...
        elseif($this->request->post['method'] == 'ebt'){
            ...
            $data['xRefNum'] = $this->session->data['sola']['xRefNum'];
        }
        ...
        if(!$output['error']) {
            ...
            if (curl_error($curl)) {
                ...
                if (($response_info['xResult'] == 'A')) {
                    $this->model_checkout_order->confirm( $orderId, $this->config->get('sola_success_order_status') );
                    $this->model_checkout_order->updatePaymentMethodData( $orderId, serialize($response_info) );
                    $this->model_checkout_order->update(
                        $orderId,
                        $this->config->get('sola_success_order_status'),
                ...
                $this->log->write('Sola CURL ERROR: Empty Gateway Response');
            }
            curl_close($curl);
        }
        ...
        if($redirect){
            if($output['success']) {
                ...
                $this->loadLanguage('sola/sola');
                $this->session->data['error'] = $this->language->get('sola_ebt_declined').'('.$response_info['xError'].': '.$response_info['xErrorCode'].')';
    ...
    public function ebt_finalize()
    {
        if(!$this->request->is_POST()
            ||
            $this->request->post['AccuId'] != $this->session->data['sola']['xAccuID']
        ){
        ...
        if($this->request->post['AccuResponseCode'] != 'ACCU000'){
            ...
            $this->session->data['error'] = sprintf(
                $this->language->get('sola_pinpad_error'),
                $this->language->get('sola_pinpad_error_'.strtolower($this->request->post['AccuResponseCode']))
            );
