...
class ControllerResponsesExtensionDefaultPPExpress extends AController
{
    ...
    public function test()
    {
        ...
        $cfg = $this->model_setting_setting->getSetting('default_pp_express', (int)$this->session->data['current_store_id']);

        if (!$cfg['default_pp_express_test']) {
        ...
        $payment_data = array(
            ...
            'VERSION'                        => '98.0',
            'USER'                           => html_entity_decode($cfg['default_pp_express_username'], ENT_QUOTES, 'UTF-8'),
            'PWD'                            => html_entity_decode($cfg['default_pp_express_password'], ENT_QUOTES, 'UTF-8'),
            'SIGNATURE'                      => html_entity_decode($cfg['default_pp_express_signature'], ENT_QUOTES, 'UTF-8'),
        ...
        if (empty($response)) {
            $warning = new AWarning('CURL Error: '.$curl_error.'. Test mode = '.$cfg['default_pp_express_test'].'.');
            ...
            $warning = new AWarning('PayPal Express Checkout Error: '.$ec_settings['L_LONGMESSAGE0'].'. Test mode = '.$cfg['default_pp_express_test'].'.');
            $warning->toLog()->toDebug();
            $test_mode = $cfg['default_pp_express_test'] ? 'ON' : 'OFF';
    ...
    public function getPublisherId()
    {
        ...
        if ($this->request->get['disable']) {
            $this->model_setting_setting->editSetting(
                'default_pp_express',
                array(
                    'default_pp_express_billmelater_publisher_id' => '',
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, AJson::encode(array(
            ...
            if ($json['publisherId']) {
                ...
                $this->model_setting_setting->editSetting(
                    'default_pp_express',
                    array(
                        'default_pp_express_billmelater_publisher_id' => $json['publisherId'],
