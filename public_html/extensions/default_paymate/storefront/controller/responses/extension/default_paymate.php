...
class ControllerResponsesExtensionDefaultPaymate extends AController
{
    ...
    public function main()
    {
        ...
        $this->data['mid'] = $this->config->get('default_paymate_username');

        $this->load->library('encryption');

        $encryption = new AEncryption($this->config->get('encryption_key'));

        $this->data['return'] = $this->html->getSecureURL('extension/paymate/callback', '&oid='.$encryption->encrypt($order_info['order_id']).'&conf='.$encryption->encrypt($order_info['payment_firstname'].$order_info['payment_lastname']));

        if ($this->config->get('default_paymate_include_order')) {
            $this->data['ref'] = html_entity_decode($this->config->get('store_name'), ENT_QUOTES, 'UTF-8')." (#".$order_info['order_id'].")";
        } else {
            $this->data['ref'] = html_entity_decode($this->config->get('store_name'), ENT_QUOTES, 'UTF-8');
        }
