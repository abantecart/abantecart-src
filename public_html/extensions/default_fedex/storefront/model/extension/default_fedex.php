        ...
        foreach ($special_ship_products as $product) {
            ...
            if ($quote_data) {
                foreach ($quote_data as $key => $value) {
                    ...
                    $quote_data[$key]['text'] =
                        $this->currency->format($quote_data[$key]['cost'], $this->currency->getCode(), 1);
                }
            } else {
                if ($new_quote_data) {
    ...
    private function _processRequest($address, $products)
    {
        ...
        if ($products) {
            ...
            foreach ($products as $product) {
                ...
                $product_width =
                    $this->length->convertByID($product['length'], $product['length_class'], $length_class_id);
                $product_length =
                    $this->length->convertByID($product['width'], $product['length_class'], $length_class_id);
                $product_height =
                    $this->length->convertByID($product['height'], $product['length_class'], $length_class_id);
        ...
        if ($priority_overnight_quote > 0 && $this->config->get('default_fedex_default_fedex_us_02') > 0) {
            ...
            $quote_data['FEDEX_PRIORITY_OVERNIGHT'] = array(
                ...
                'cost'         => $this->currency->convert($priority_overnight_quote, 'USD',
                    $this->currency->getCode()),
                'tax_class_id' => 0,
