...
class ExtensionAvataxIntegration extends Extension{
	...
	public function onControllerCommonListingGrid_InitData(){
		...
		if ($that->data['table_id'] == 'product_grid') {
			...
			$that->data['actions']['edit']['children']['avatax_integration'] = array (
					...
					'href' => $that->html->getSecureURL('catalog/avatax_integration', '&product_id=%ID%'),
			);
		}
	}
	...
	public function onControllerPagesCatalogProductTabs_InitData(){
		...
		$this->data['tabs'][] = array (
				'href'   => $that->html->getSecureURL('catalog/avatax_integration',
						'&product_id=' . $that->request->get['product_id']),
				'text'   => $that->language->get('avatax_integration_name'),
				'active' => $that->data['active'] == 'avatax_integration' ? true : false
		);
		...
		if ($this->baseObject_method == 'details') {
			...
			if ($order['order_status_id'] == $that->config->get('avatax_integration_status_success_settled')
				...
				$that->view->addHookVar('order_details',
						'<div class="alert alert-danger" role="alert">Avatax is already calculated and documented. Edits to this order will not be reflected on Avatax!</div>');
			}
		}
	}
	...
	public function onControllerPagesSaleOrder_InitData(){
		...
		if ($this->baseObject_method == 'history') {
			...
			if (isset($order_id) && isset($status_id) && $status_id == $that->config->get('avatax_integration_status_return_settled')) {
				$that->load->model('sale/order');
				$order = $that->model_sale_order->getOrder($order_id);
				$order_totals = $that->model_sale_order->getOrderTotals($order_id);
				$customer_id = $order['customer_id'];
			...
			if (isset($order_id) && isset($status_id) && $status_id == $that->config->get('avatax_integration_status_cancel_settled')) {
				$that->load->model('sale/order');
				$order = $that->model_sale_order->getOrder($order_id);
				$order_totals = $that->model_sale_order->getOrderTotals($order_id);
				$customer_id = $order['customer_id'];
	...
	public function onControllerResponsesListingGridOrder_UpdateData(){
		...
		if ($this->baseObject_method == 'update_field') {
			...
			if (isset($order_id) && isset($status_id) && $status_id == $that->config->get('avatax_integration_status_success_settled')) {
				$that->load->model('sale/order');
				$order = $that->model_sale_order->getOrder($order_id);
				$order_totals = $that->model_sale_order->getOrderTotals($order_id);
				$customer_id = $order['customer_id'];
	...
	public function onControllerPagesCheckoutGuestStep3_UpdateData(){
		...
		if ($that->config->get('avatax_integration_status')
			...
			if ($res['error']) {
				$that->loadLanguage('avatax_integration/avatax_integration');
				$that->view->assign('error_warning',
		...
		if (isset($that->session->data['order_id'])) {
			$this->_set_order_product_tax_codes($that->session->data['order_id']);
		}
	}

	public function _set_order_product_tax_codes($order_id){
		$that = $this->baseObject;
	...
	public function onControllerPagesCheckoutConfirm_UpdateData(){
		...
		if ($that->config->get('avatax_integration_status')
			...
			if(!$that->customer->isLogged() &&  $that->session->data['guest']) {
				...
				$address_id = $that->session->data['shipping_address_id'] ? $that->session->data['shipping_address_id'] : $that->session->data['payment_address_id'];
	...
	public function _calc_total_discount($total_data){
		$total_discount = 0;
	...
	public function getTax($that, $cust_data, $commit = false, $total_data = 0, $return = false){
		...
		if (!empty($serviceURL) && !empty($accountNumber) && !empty($licenseKey)) {
			...
			$customer_settings = $that->model_extension_avatax_integration->getCustomerSettings($cust_data['customer_id']);
			if($order_id) {
				...
				elseif ( $config->get('avatax_integration_commit_documents')
						...
						&& (!$customer_settings['exemption_number'] || $customer_settings['status'] == 1 && $customer_settings['exemption_number'])
				){
					$getTaxRequest->setCommit(true);
				} else {
			...
			if (is_array($total_data)) {
				$getTaxRequest->setDiscount($this->_calc_total_discount($total_data));
			}
...
// Address Data
			...
			if (isset($order_data['shipping_postcode']) && !empty($order_data['shipping_postcode']) && $config->get('config_tax_customer') == 0) {
			...
			} elseif (isset($order_data['payment_postcode']) && !empty($order_data['payment_postcode']) && $config->get('config_tax_customer') == 1) {
			...
			if ($order_id) {
				...
				foreach ($product_data as $key => $values) {
					...
					$line->setTaxCode($that->model_extension_avatax_integration->getProductTaxCode($values['product_id']));
					$lines[] = $line;
					$counter++;
				}
...
//Get Results                
	...
	public function onControllerPagesExtensionExtensions_UpdateData(){
		...
		if (IS_ADMIN && $current_ext_id == 'avatax_integration' && $this->baseObject_method == 'edit') {
			$html = '<a class="btn btn-white tooltips" target="_blank" href="http://www.avalara.com/integrations/abantecart" title="Visit Avalara">
	...
	public function onControllerResponsesListingGridExtension_UpdateData(){
		...
		if ($this->baseObject_method == 'update') {
			if (isset($that->request->post['avatax_integration']['avatax_integration_status'])) {
				...
				$that->loadModel('setting/setting');
				$activateTotalArray = array ('avatax_integration_total' => array ('avatax_integration_total_status' => $avatax_integration_total_status));
				foreach ($activateTotalArray as $group => $values) {
					$that->model_setting_setting->editSetting($group, $values);
				}
			}
	...
	public function onControllerPagesAccountEdit_InitData() {
		...
		if ( $that->request->is_POST() && $that->request->post['exemption_number']) {
			...
			if(in_array($customer_settings['status'], array(0,2))){
				...
				$that->messages->saveNotice($that->language->get('avatax_integration_review_number_title'),
											sprintf($that->language->get('avatax_integration_review_number_message'), $that->customer->getId()),
											false);
			}
		}
	}
	...
	public function onControllerPagesAccountCreate_UpdateData() {
		...
		if ( $that->request->is_POST() && $that->request->post['exemption_number'] && !$that->errors) {
			$customer_settings = $that->model_extension_avatax_integration->getCustomerSettings( $that->customer->getId() );
			if(in_array($customer_settings['status'], array(0,2))){
					$that->model_extension_avatax_integration->setCustomerSettings(
					$that->customer->getId(),
