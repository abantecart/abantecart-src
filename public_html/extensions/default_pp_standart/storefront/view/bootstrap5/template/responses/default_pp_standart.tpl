<form action="<?php
echo str_replace('&', '&amp;', $action); ?>" method="post" id="checkout" <?php
echo $target_parent; ?>>
    ...
    foreach ($products as $product) { ?>
        ...
        <input type="hidden" name="weight_<?php
        echo $i; ?>" value="<?php
        echo $product['weight']; ?>"/>
        <?php
        $j = 0;
    ...
    echo $address1; ?>"/>
    <input type="hidden" name="address2" value="<?php
    echo $address2; ?>"/>
    ...
    echo $cancel_return; ?>"/>
    <input type="hidden" name="paymentaction" value="<?php
    echo $paymentaction; ?>"/>
    ...
    <?php
    if ($logo_url) { ?>
        <input type="hidden" name="image_url" value="<?php
        echo $logo_url; ?>"/>
    <?php
    }
    if ($cartbordercolor) { ?>
        <input type="hidden" name="cpp_cart_border_color" value="<?php
        echo $cartbordercolor; ?>"/>
    <?php
    } ?>
    ...
    <div class="form-group action-buttons text-center">
        ...
        echo $back->href; ?>" class="btn btn-default mr10" title="<?php
        echo $back->text ?>">
        ...
        <button id="<?php
        echo $button_confirm->name ?>" class="btn btn-orange lock-on-click" title="<?php
        echo $button_confirm->name ?>" type="submit">
            <i class="fa fa-check"></i>
            <?php
            echo $button_confirm->name; ?>
        </button>
    </div>
</form>