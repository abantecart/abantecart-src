...
class ModelExtensionDefaultFreeShipping extends Model {
	function getQuote($address) {
		...
		if ($this->config->get('default_free_shipping_status')) {
      		$query = $this->db->query("SELECT * FROM " . $this->db->table("zones_to_locations") . " WHERE location_id = '" . (int)$this->config->get('default_free_shipping_location_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
