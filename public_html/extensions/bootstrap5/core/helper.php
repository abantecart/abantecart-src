...
function preparePreset($data, $mode, $indexes){
    if($mode == 'html'){
        $doc = new DOMDocument();
        $doc->loadHTML($data);
        ...
        $output = processComponent($data, $indexes);
    }

    return $output;
}

function processComponent($data, $indexes){
    foreach($data as &$item){
        ...
        if($item['components']){
            $item['components'] = processComponent($item['components'], $indexes);
        }
    }
