...
class ControllerResponsesExtensionDefaultWorldPay extends AController
{
    ...
    public function main()
    {
        ...
        $test = $this->config->get('default_worldpay_test');
        if ($test == 0) {
        ...
        $this->data['MC_callback'] = $this->html->getSecureURL('r/extension/default_worldpay/callback');
        $md5_secret_key = $this->config->get('default_worldpay_md5secret');
        //  signature fields:  currency:amount:testID:instId
        $signature_fields = $order_info['currency']
            .":".$this->data['amount']
            .":".$this->config->get('default_worldpay_test')
            .":".$this->config->get('default_worldpay_merchant');
        $this->data['signature'] = md5($md5_secret_key.":".$signature_fields);
    ...
    public function callback()
    {
        if (isset($this->request->post['callbackPW']) && ($this->request->post['callbackPW'] == $this->config->get('default_worldpay_password'))) {
            ...
            if (isset($this->request->post['transStatus']) && $this->request->post['transStatus'] == 'Y') {
                ...
                $this->data['orderid'] = $this->request->post['cartId'];

                $message = '';
                if (isset($this->request->post['transId'])) {
                    $message .= 'WorldPay transaction Id: '.$this->request->post['transId'];
                }
                if (isset($this->request->post['transStatus'])) {
                    $message .= 'Transaction Status: '.$this->request->post['transStatus'];
                }
                ...
                $template = 'extension/default_worldpay_failure.tpl';
            }
            $this->view->batchAssign($this->data);
