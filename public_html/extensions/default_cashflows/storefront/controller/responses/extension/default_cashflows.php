...
class ControllerResponsesExtensionDefaultCashflows extends AController
{
    public function main()
    {
        ...
        //build submit form
        $form = new AForm();
        $form->setForm(['form_name' => 'cashflows']);
        ...
        //load credit card input validation
    ...
    public function send()
    {
        ...
        $payment_data = [
            ...
            'card_start'    => $this->request->post['cc_start_date_month']
                .substr(
                    $this->request->post['cc_start_date_year'], 2
                ),
            'card_expiry'   => $this->request->post['cc_expire_date_month']
                .substr(
        ...
        if ($response) {
            ...
            if (isset($data[0]) && $data[0] == 'A') {
                ...
                if (isset($data[2])) {
                    if ($data[2] == '232') {
                        $message .= $this->language->get('text_avs')
                            .' '
                            .$this->language->get('text_avs_full_match')
                            ."\n";
                    } elseif ($data[2] == '400') {
                        $message .= $this->language->get('text_avs')
                            .' '
                            .$this->language->get('text_avs_not_match')
                            ."\n";
                    }
                }
                ...
                $this->model_checkout_order->update(
                    $this->session->data['order_id'],
                    $this->config->get('default_cashflows_order_status_id'),
                    $message,
                    false
                );
