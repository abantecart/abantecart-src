<div class="thumbnails grid row list-inline">
    ...
    foreach ($products as $product) {
        ...
        $item['rating'] = ($product['rating']) ? "<img class=\"rating\" src='".$this->templateResource('/image/stars_'
                .$product['rating'].'.png')."' alt='".$product['stars']."' width='64' height='12' />" : '';
...
<div class="thumbnails list row">
    ...
    foreach ($products as $product) {
        ...
        $item['title'] = $product['name'];
        $item['rating'] = ($product['rating']) ? "<img class=\"rating\" src='".$this->templateResource('/image/stars_'
                .$product['rating'].'.png')."' alt='".$product['stars']."' />" : '';
                            ...
                            <div class="pricetag pull-right">
                                ...
                                <?php } else {
                                    if ($product['track_stock'] && !$product['in_stock']) { ?>
                                        <span class="nostock"><?php echo $product['no_stock_text']; ?></span>
                                    <?php } else { ?>
                                        <?php if (!$this->getHookVar('product_add_to_cart_list_html_'
                                            .$product['product_id'])) { ?>
                                            <a data-id="<?php echo $product['product_id'] ?>"
                                               href="<?php echo $item['buy_url'] ?>"
                                               class="productcart"><?php echo $button_add_to_cart ?></a>
                                        <?php } else { ?>
                                            <?php echo $this->getHookVar('product_add_to_cart_list_html_'
                                                .$product['product_id']); ?>
                                        <?php } ?>
                                    <?php }
                                } ?>
