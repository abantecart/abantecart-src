...
if ($product['rating']) {
    ...
    html = '<?php echo '<img src="'.AUTO_SERVER.$this->templateResource('/image/stars_'.(int)$product['rating'].'.png')
        .'" alt="'.$product['stars'].'" />' ?>';
...
if ($product['quantity'] && !($product['track_stock'] && !$product['in_stock']) && !$product['call_to_order']) {
    ...
    html = '<span class="abantecart_quantity_text"><?php echo $text_qty; ?></span>&nbsp;<input type="text" size="3"
                                                                                               class="abantecart_quantity_field"
                                                                                               placeholder="<?php echo $text_qty; ?>"
                                                                                               value="<?php echo $product['quantity']->value ?>"
                                                                                               id="product_quantity"
                                                                                               name="<?php echo $product['quantity']->name ?>"></div>';
...
if ($product['button_addtocart']) {
    ...
    <?php } else { ?>
        html ='
        <button <?php echo $product['button_addtocart']->attr; ?>
                title="<?php echo $product['button_addtocart']->text; ?>" class="abantecart_button"
                id="<?php echo $product['button_addtocart']->id; ?>"
                type="button"><?php echo $product['button_addtocart']->text; ?></button>';
    <?php } ?>
