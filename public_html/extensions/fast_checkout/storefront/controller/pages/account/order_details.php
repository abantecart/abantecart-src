...
class ControllerPagesAccountOrderDetails extends AController
{
    ...
    public function main()
    {
            ...
            if (isset($this->request->get['ot']) && $this->config->get('config_guest_checkout')) {
                ...
                if (!$this->customer->isLogged() && $order_id) {
                    $this->session->data['redirect'] =
                        $this->html->getSecureURL('account/order_details', '&order_id='.$order_id);
        ...
        if ($order_info) {
            ...
            foreach ($order_products as $product) {
                ...
                $products[] = array(
                    ...
                    'price'     => $this->currency->format($product['price'], $order_info['currency'],
                        $order_info['value']),
                    'total'     => $this->currency->format($product['total'], $order_info['currency'],
                        $order_info['value']),
                );
            }
            ...
            foreach ($results as $result) {
                $histories[] = array(
                    'date_added' => dateISO2Display($result['date_added'],
            ...
            if ($this->config->get('config_customer_cancelation_order_status_id')) {
                ...
                if (in_array($order_info['order_status_id'], $order_cancel_ids)) {
                    ...
                    if (!$guest) {
                        $this->data['order_cancelation_url'] =
    ...
    protected function _build_download_list($order_id)
    {
        ...
        if ($this->data['guest']) {
            ...
            $customer_downloads =
                $this->model_extension_fast_checkout->getCustomerOrderDownloads($order_id, $this->customer->getId());
        }
        ...
        foreach ($customer_downloads as $download_info) {
            ...
            $downloads[] = array(
                ...
                'date_added'  => dateISO2Display($download_info['date_added'],
                ...
                'text'        => $download_text,
                'expire_date' => dateISO2Display($download_info['expire_date'],
            ...
            if ($order_download_id && $this->customer->isLogged()) {
                ...
                if (!$this->customer->isLogged() && isset($this->request->get['ot'])
                    ...
                    if ($order_token) {
                        ...
                        if ($order_id && $email) {
                            $order_downloads =
                                $this->model_extension_fast_checkout->getCustomerOrderDownloads($order_id, 0);
                            if ($order_downloads) {
                                //check is customer has requested download in his order
