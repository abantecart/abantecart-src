...
<form id="perpetual" class="form-horizontal validate-creditcard">
...
function confirmSubmit() {		
	$.ajax({
		...
		beforeSend: function() {
			$('.alert').remove();
			$('#perpetual .action-buttons').hide(); 
			$('#perpetual .action-buttons').before('<div class="wait alert alert-info text-center"><i class="fa fa-refresh fa-spin"></i> <?php echo $text_wait; ?></div>');
		},
		success: function(data) {
			if (!data) {
				$('.wait').remove();
				$('#perpetual .action-buttons').show(); 
				$('#perpetual').before('<div class="alert alert-danger"><i class="fa fa-bug"></i> <?php echo $error_unknown; ?></div>');
			} else {					  			
				if (data.error) {
					$('.wait').remove();
					$('#perpetual .action-buttons').show(); 
					$('#perpetual').before('<div class="alert alert-warning"><i class="fa fa-exclamation"></i> '+data.error+'</div>');
				}	
				if (data.success) {			
		...
		error: function (jqXHR, textStatus, errorThrown) {
			$('.wait').remove();
			$('#perpetual .action-buttons').show(); 
			$('#perpetual').before('<div class="alert alert-danger"><i class="fa fa-exclamation"></i> '+textStatus+' '+errorThrown+'</div>');
		}				
	});
}
//--></script>