...
class ACacheDriverAPC extends ACacheDriver{
	...
	public function get($key, $group, $check_expire = true){
		$cache_key = $this->_hashCacheKey($key, $group);
	...
	public function clean($group){
		...
		foreach ($keys as $key){
			...
			if (strpos($internalKey, $this->secret . '-cache-' . $group . '-') === 0 ){
	...
	public function lock($key, $group, $locktime){
		...
		$cache_key = $this->_hashCacheKey($key, $group).'_lock';
	...
	public function unlock($key, $group = null) {
		return apc_delete($this->_hashCacheKey($key, $group).'_lock');
	}
}