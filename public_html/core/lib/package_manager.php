...
class APackageManager {
	...
	public function getRemoteFileHeaders( $url) {
		...
		$result =  $file->getDataHeaders($url.'&file_size=1');
		if ( ! $result ) {
			$this->error = $file->error;
			return false;
		}
	...
	public function replaceCoreFiles(){
		...
		if($this->session->data['package_info']['ftp']){
			foreach($corefiles as $core_filename){
				...
				$src_dir = $this->session->data['package_info']['tmp_dir'].$this->session->data['package_info']['package_dir'].'/code/'.$core_filename;
