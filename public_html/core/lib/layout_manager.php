...
class ALayoutManager {
	...
	private function _processCustomBlock($layout_id, $block, $parent_instance_id = 0) {
		...
		// get custom block
		//get custom block info of banner slider extension
		$custom_block_info = $this->getBlocksList(array( 'subsql_filter' => "bd.name = '" . (string)$block->custom_block_txt_id . "'" ));
		$custom_block_id = $custom_block_info[ 0 ][ 'custom_block_id' ];

		$action = (string)$block->action;
		if (in_array($action, array( "", null, "update" ))) {
			$action = 'insert';
		}
		...
		if ($action == 'delete') {
			...
			//If exists same block with same block_txt_id, return error and finish
			if ($custom_block_id) {
				$errmessage = 'Error: layout.xml::cannot insert custom block (custom_block_txt_id: "' . $block->custom_block_txt_id . '") into database because it already exists.';
				$error = new AError ($errmessage);
				$error->toLog()->toDebug();
				$this->errors = 1;
