...
final class ALanguage {
	...
	public function definitionAutoLoad($language_id, $section, $filename, $mode = 'add', $language_key = '') {
		...
		if ($mode == 'update') {
			...
			foreach ($language_ids as $lang_name => $lang_id) {
				...
				//load languages into db for (admin/storefront)
				foreach ($sections as $sect) {
					...
					foreach ($language_blocks[ 'extensions' ][ $sect ] as $rt) {
						...
						ADebug::checkpoint('ALanguage loading ' . $sect . '-' . $rt . ' load start');
						$alang->load($rt, 'silent');
						ADebug::checkpoint('ALanguage loading ' . $sect . '-' . $rt . ' load end');
						if ($language_key) {
					...
					foreach ($language_blocks[ $sect ] as $rt) {
						ADebug::checkpoint('ALanguage loading ' . $sect . '-' . $rt . ' start');
						$alang->load($rt, 'silent');
						ADebug::checkpoint('ALanguage loading ' . $sect . '-' . $rt . ' load end');
						if ($language_key) {
							$alang->get($language_key);
			...
			foreach ($language_ids as $lang_name => $lang_id) {
				...
				foreach ($sections as $sect) {
					...
					foreach ($language_blocks[ 'extensions' ][ $sect ] as $rt) {
						...
						$alang->load($rt, 'silent');
						ADebug::checkpoint('ALanguage read xml file ' . $rt . ' start');
						$lang_keys = $alang->ReadXmlFile($rt);
						ADebug::checkpoint('ALanguage read xml file ' . $rt . ' stop');
						if ($language_key && in_array($language_key, array_keys($lang_keys))) {
							$alang->get($language_key);
						} else {
							ADebug::checkpoint('ALanguage loading language keys for ' . $rt . ' start');
							foreach ($lang_keys as $k => $v) {
								$alang->get($k);
							}
							ADebug::checkpoint('ALanguage loading language keys for ' . $rt . ' stop');
						}
					}
	...
	public function getAllLanguageBlocks($language_name = 'english') {
		...
		// admin
		$lang_dir = (defined(INSTALL) ? DIR_ABANTECART : DIR_ROOT . '/') . 'admin/language/' . $language_name;
		$xml_files = getFilesInDir($lang_dir, 'xml');
		foreach ($xml_files as $file) {
			$result[ 'admin' ][ ] = str_replace('.xml', '', str_replace($lang_dir . '/', '', $file));
		}
		//storefront
		$lang_dir = (defined(INSTALL) ? DIR_ABANTECART : DIR_ROOT . '/') . 'storefront/language/' . $language_name;
		$xml_files = getFilesInDir($lang_dir, 'xml');
		foreach ($xml_files as $file) {
