<?php
/**
* 2011-2015 AbanteCart, Ideal OpenSource Ecommerce Solution
...
class AAttribute
{
	...
	private $core_attribute_types_controllers = array(
		'responses/catalog/attribute/getProductOptionSubform',												'responses/catalog/attribute/getDownloadAttributeSubform'
		);
	...
	private function _load_attributes($attribute_type_id, $language_id = 0)
	{
        ...
        $query = $this->db->query('
			...
			AND ga.`status` = 1 ORDER BY ga.`sort_order`'
		);
	...
	public function getActiveAttributeGroup($group_id, $language_id = 0)
	{
        ...
        $query = $this->db->query('
			...
			LEFT JOIN `'.$this->db->table('lobal_attributes_groups_descriptions').'` gagd
	...
	public function getAttributeValues($attribute_id, $language_id = 0)
	{
        ...
        $query = $this->db->query('
            ...
            WHERE gav.`attribute_id` = \''.$this->db->escape($attribute_id).'\' order by gav.`sort_order`'
        );
