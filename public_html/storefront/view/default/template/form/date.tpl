...
<?php } ?>
    <input type="<?php echo $type ?>"
           name="<?php echo $name ?>"
           id="<?php echo $id ?>"
           value="<?php echo $value; ?>" <?php echo $attr; ?> class="form-control <?php echo $style; ?>" <?php if ( $required ) { echo 'required'; }?>/>
    <?php if ( $required ) { ?>
    <span class="input-group-text text-danger">*</span>
    <?php }else{ ?>
        <div class="reset-date input-group-addon">
            <button id="reset_date_<?php echo $id ?>" class="btn btn-default btn-xs" type="button"
