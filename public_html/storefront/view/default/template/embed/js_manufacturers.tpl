...
<?php foreach($manufacturers as $manufacturer){
	...
	if($('#<?php echo $target;?> .abantecart_name')){
		$('#<?php echo $target;?> .abantecart_name').html('<?php echo $manufacturer['name']?>');
	}

	if($('#<?php echo $target;?> .abantecart_image')){
		html = '<a data-href="<?php echo $manufacturer['details_url'];?>"  data-id="<?php echo $manufacturer['manufacturer_id']; ?>" data-html="true" data-target="#abc_embed_modal" data-toggle="abcmodal" href="#" class="category_thumb" data-original-title="">'
