...
var html = '';
if($('#<?php
echo $target; ?> .abantecart_name')){
$('#<?php
echo $target; ?> .abantecart_name').html('<?php
echo $product['name'] ?>');
}

if($('#<?php
echo $target; ?> .abantecart_image')){
html = '<a data-href="<?php
echo $product_details_url; ?>" data-id="<?php
...
+ '<?php
echo nl2br($product['thumbnail']['thumb_html']); ?></a>';
$('#<?php
echo $target; ?> .abantecart_image').html(html);
}
...
if ($product['price'] && $display_price) {
    ...
    $('#<?php
    echo $target; ?> .abantecart_price').html(html);
    }
    <?php
}
...
if($product['rating']) {
    ...
    echo '<img src="'
        ...
        .'" alt="'.$product['stars'].'" />' ?>';
    $('#<?php
    echo $target; ?> .abantecart_rating').html(html);
    }
    <?php
}
...
if($product['quantity'] && !($product['track_stock'] && !$product['in_stock']) && !$product['call_to_order']) {
    ...
    html ='';
    if($('<?php
    echo $target; ?> .abantecart_quantity')){
    html = '<span class="abantecart_quantity_text"><?php
        js_echo($text_qty); ?></span>'
    + '&nbsp;<input type="text" size="3" class="abantecart_quantity_field" '
    + 'placeholder="<?php
    js_echo($text_qty); ?>" value="<?php
    echo $product['quantity']->value ?>" '
    + 'id="product_quantity" name="<?php
    echo $product['quantity']->name ?>"></div>';
    $('#<?php
    echo $target; ?> .abantecart_quantity').html(html);
    }
<?php
}
