...
foreach ((array) $products as $product) { ?>
    if($('#<?php echo $target; ?> .abantecart_collection ul')) {
        ...
        html += '<div><h3 class="abantecart_product_name"><a data-href="<?php echo $product['product_details_url']; ?>" '
    ...
    <?php
        ...
        if ($product['rating']) { ?>
            html += '<div class="rating"><img src="<?php echo AUTO_SERVER.$this->templateResource('/image/stars_'.(int) $product['rating'].'.png'); ?>" alt="<?php echo $product['stars']; ?>"/></div>';
    <?php
        } ?>
        html += '</li>';
