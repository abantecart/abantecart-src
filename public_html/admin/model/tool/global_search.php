...
class ModelToolGlobalSearch extends Model {
	...
	public function getSearchSources($keyword = '') {
		...
		if (mb_strlen ( $keyword, strtoupper ( $this->registry->get ( 'document' )->getCharset () ) ) >= 1) {
			$search_categories = array ( "orders"=>'order',
										 "customers"=>'customer',
	 ...
	 * @return string(json)
	 */
	public function getResult($search_category, $keyword) {
		$this->registry = Registry::getInstance ();
		$db = $this->registry->get ( 'db' );
		$this->charset = strtoupper ( $this->registry->get ( 'document' )->getCharset () );
		$needle = $db->escape ( strtolower ( htmlentities ( $keyword, ENT_QUOTES, $this->charset ) ), $this->charset );
		...
		switch ($search_category) {
			case 'product_categories' :
				...
				$result = $db->query ( $sql );
				$result = $this->prepareResponse ( $keyword, 'catalog/category/update', "category_id", $result->rows );
				break;
			
			case 'languages' :
				$sql = "SELECT l.language_definition_id, CONCAT_WS(' >> ',l.block,l.language_key) as title, CONCAT_WS(' >> ',l.language_key,l.language_value) as text
						FROM " . DB_PREFIX . "language_definitions l						
						WHERE (LOWER(l.language_value) like '%" . $needle . "%')
				...
				$result = $db->query ( $sql );
				$result = $this->prepareResponse ( $keyword, 'localisation/language_definitions/update', "language_definition_id", $result->rows );
			...
			case 'products' :
				...
				$sql = "SELECT a.product_id, b.name as title, a.model as text
						...
						WHERE LOWER(pd1.name) like '%" . $needle . "%' AND pd1.language_id IN (".(implode(",",$search_languages)).")						
						UNION
						 						
						SELECT pd2.product_id, pd2.name as title, pd2.meta_keywords as text
						FROM " . DB_PREFIX . "product_descriptions pd2
						WHERE LOWER(pd2.meta_keywords) like '%" . $needle . "%' AND pd2.language_id IN (".(implode(",",$search_languages)).")						
						UNION 						
						SELECT pd3.product_id, pd3.name as title, pd3.meta_description as text
						FROM " . DB_PREFIX . "product_descriptions pd3
						WHERE LOWER(pd3.meta_description) like '%" . $needle . "%' AND pd3.language_id IN (".(implode(",",$search_languages)).")						
						UNION 						
						SELECT pd4.product_id, pd4.name as title, pd4.description as text
						FROM " . DB_PREFIX . "product_descriptions pd4
						WHERE LOWER(pd4.description) like '%" . $needle . "%' AND pd4.language_id IN (".(implode(",",$search_languages)).")						
						UNION 						
						...
						LEFT JOIN " . DB_PREFIX . "product_descriptions b 
							ON (b.product_id = a.product_id AND b.language_id	IN (".(implode(",",$search_languages))."))
						WHERE LOWER(a.tag) like '%" . $needle . "%' AND a.language_id IN (".(implode(",",$search_languages)).")
						LIMIT " . $offset . "," . $rows_count;
				...
				$result = $this->prepareResponse ( $keyword, 'catalog/product/update', "product_id", $table );
			...
			case "reviews" :
				$sql = "SELECT review_id, r.`text`, pd.`name` as title
						...
						WHERE (LOWER(r.`text`) LIKE '%" . $needle . "%') OR (LOWER(r.`author`) LIKE '%" . $needle . "%')
						LIMIT " . $offset . "," . $rows_count;
				
				$result = $db->query ( $sql );
				$result = $this->prepareResponse ( $keyword, 'catalog/review/update', "review_id", $result->rows );
			...
			case "manufacturers" :
				...
				$result = $db->query ( $sql );
				$result = $this->prepareResponse ( $keyword, 'catalog/manufacturer/update', "manufacturer_id", $result->rows );
			...
			case "orders" :
				...
				$result = $db->query ( $sql );
				$result = $this->prepareResponse ( $keyword, 'sale/order/update', "order_id", $result->rows );
				
				break;
				
			case "customers" :
				$sql = "SELECT customer_id, CONCAT('customer: ', firstname,' ',lastname) as title,				
						...
						LIMIT " . $offset . "," . $rows_count;				
				$result = $db->query ( $sql );				
				$result = $this->prepareResponse ( $keyword, 'sale/customer/update', "customer_id", $result->rows );
			...
			case "pages" :
				...
				$result = $db->query ( $sql );
				$result = $this->prepareResponse ( $keyword, 'design/layout', array('page_id','layout_id','tmpl_id'), $result->rows );
			...
			case "settings" :
				...
				$result = $db->query ( $sql );
				$result = $this->prepareResponse ( $keyword, 'setting/setting', array("setting_id","active"), $result->rows );
			...
			case "messages" :
				...
				$result = $db->query ( $sql );
				$result = $this->prepareResponse ( $keyword, 'tool/message_manager', "msg_id", $result->rows );
			...
			case "extensions" :
				...
				$result = $db->query ( $sql );
				$result = $this->prepareResponse ( $keyword, 'extension/extensions/edit', "extension", $result->rows );
	 ...
	 * @return array
	 */
	private function prepareResponse($keyword = '', $rt = '', $key_field = '', $table = array()) {
		$output = array ();
