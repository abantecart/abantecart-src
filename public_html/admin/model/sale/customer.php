...
class ModelSaleCustomer extends Model {
	...
	public function getTransaction($transaction_id=0){
		if(!(int)$transaction_id) return array();
		...
		$sql = "SELECT *,
				CASE
					...
					ELSE
						''
					 END as user
				FROM " . $this->db->table("transactions") . " t
				LEFT JOIN " . $this->db->table("users") . " u ON u.user_id = t.created_by
				WHERE t.transaction_id = '" . (int)$transaction_id . "'";
		$result = $this->db->query($sql);
		$row = $result->row;
		if(empty($row['user']) && $row['section']!=1){
			$customer_info = $this->getCustomer($row['customer_id']);
			$row['user'] = $customer_info['firstname'].' '.$customer_info['lastname'];
		}
		return $row;
	}

	public function getTransactions($data=array(), $mode=''){
		// get decrypted customer name first
		$customer_info = $this->getCustomer((int)$data['customer_id']);
		$sql = "SELECT *,
				CASE
					WHEN t.section=1
						THEN CONCAT(u.firstname,' ',u.lastname, ' (',u.username,')')
		...
		if (has_value($filter['type'])) {
			$implode[] = "t.type = '" . $this->db->escape($filter['type']) . "'";
		}
	...
	public function getTotalTransactions($data){
		return $this->getTransactions($data,'total_only');
	}
