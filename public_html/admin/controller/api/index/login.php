...
class ControllerApiIndexLogin extends AControllerAPI
{
        ...
        if (isset($request['token'])) {
            ...
            */
            ...
            if (isset($request['username']) &&
                ...
                $this->validateUsser($request['username'], $request['password'])) {
                if (!session_id()) {
    ...
    private function validateUsser($username, $password)
    {
        if (isset($username) && isset($password) && !$this->user->login($username, $password)) {
            $this->loadLanguage('common/login');
            $this->messages->saveNotice(
                'API '.$this->language->get('error_login_message').$this->request->getRemoteIP(),
                $this->language->get('error_login_message_text').$username
            );
            return false;
        } else {
            return true;
