...
class ControllerPagesToolImportUpload extends AController
{
     ...
     * @var array()
     */
    public $file_types = ['text/csv', 'application/vnd.ms-excel', 'text/plain', 'application/octet-stream'];
    /**
     * @var AData
     */
    private $handler;
    ...
    protected function _prepare_import()
    {
        ...
        $res = [];
        $res['run_mode'] = $post['test_mode'] ?? 'commit';
        $res['delimiter_id'] = $post['options']['delimiter'];
        $res['delimiter'] = $this->handler->csvDelimiters[$res['delimiter_id']];

        if (in_array($file['type'], ['text/csv', 'application/vnd.ms-excel', 'text/plain', 'application/octet-stream'])) {
    ...
    protected function validateRequest()
    {
        ...
        } elseif (file_exists($file['tmp_name']) && !$file['size']) {
