...
class ControllerPagesCatalogProduct extends AController
{
    ...
    public function main()
    {
        ...
        $grid_settings = array(
            ...
            /**
             *
             * NOTE: set default sorting by date_modified,
             * but column not present in the grid
             *
            */
        ...
        $form = new AForm();
        $form->setForm(array('form_name' => 'product_grid_search'));
    ...
    public function copy()
    {
        ...
        if (isset($this->request->get['product_id']) && $this->validateCopy()) {
            ...
            if ($this->data['new_product']) {
                $this->session->data['success'] = sprintf($this->language->get(
                    'text_success_copy'
    ...
    private function getForm($args = array())
    {
        ...
        $this->document->addBreadcrumb(array(
            'href' => $this->html->getSecureURL('catalog/product'),
            'text' => ($product_id ? $this->language->get(
                'text_edit'
            )."&nbsp;".$this->language->get(
                'text_product'
            ).'-'.$this->data['product_description'][$content_language_id]['name'] : $this->language->get(
                'text_insert'
            )),
            'separator' => '::',
            'current' => true
        ...
        /* Cali-gula improve it on next branch */
        ...
        /* Cali-gula improve it on next branch - task: create file tools abstrate mode */
        $fields = array(
            'product_category',
            'featured',
            'product_store',
        ...
        $config_stock_status_id = $this->config->get('config_stock_status_id');
        $stock_status_id = (int)$this->data['stock_status_id'];

        $this->data['form']['fields']['data']['stock_status'] = $form->getFieldHtml(array(
            'type' => 'selectbox',
            'name' => 'stock_status_id',
            'value' => has_value($this->data['stock_status_id']) ? $stock_status_id : $config_stock_status_id,
        ...
        /* oh! QUO VADIS => Cali-gula  =>  what is it */
    ...
    protected function validateForm()
    {
        ...
        if ($error_text = $this->html->isSEOkeywordExists('product_id='.$product_id, $keyword)) {
