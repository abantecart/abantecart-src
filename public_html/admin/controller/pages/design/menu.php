...
class ControllerPagesDesignMenu extends AController {
	...
	public function insert() {
		...
		if ( $this->request->is_POST() && $this->_validateForm()) {
			...
			foreach ($languages as $l) {
				if ($l['language_id'] == $language_id){ continue; }
				$this->request->post['item_text'][$l['language_id']] = $this->request->post['item_text'][$language_id];
			}
			...
			$result = $this->menu->insertMenuItem(array(
					...
					'item_icon_rl_id' => $this->request->post ['item_icon_resource_id'],
	...
	public function update() {
		...
		if (($this->request->is_POST()) && $this->_validateForm()) {
			...
			$item_keys = array('item_icon', 'item_text', 'item_url', 'parent_id', 'sort_order', 'item_icon_resource_id');
	...
	private function _getForm() {
		...
		$this->data['form']['fields']['item_id'] = $form->getFieldHtml(array(
				'type' => 'input',
				'name' => 'item_id',
				'value' => $this->data ['item_id'],
				'required' => true,
				'attr' => isset ($this->request->get ['item_id']) ? 'disabled' : ''));
	...
	private function _validateForm() {
		...
		if (empty ($this->request->post ['item_text'][$this->language->getContentLanguageID()])) {
