...
<script type="text/javascript">
    ...
    var sideDialog = function (type, action, id) {
        ...
        if (id) {
            src += '&resource_id=' + id;
            //highlight resource in the list
            ...
            success: function (json) {
                ...
                $(json.items).each(function (index, item) {
                    ...
                    html += '<div class="center thumbnail" id="image_row' + item['resource_id']
                        + '" ><a class="btn resource_edit" ' + data_mode
                        + ' data-type="' + type + '" data-rl-id="' + item['resource_id'] + '">'
                        + src + '</a></div>';
                    ...
                    if (json.object_id) {
                        ...
                        html += '<a class="btn resource_edit tooltips" ' +
                            ...
                            'data-original-title="<?php echo_html2view(
                                $button_edit
                            ...
                            'onclick="unmap_resource(' + item['resource_id'] + ',\'' + json.object_name +
                            '\',\'' + json.object_id + '\');"><i class="fa fa-unlink"></i></a>';
                ...
                if (json.total > json.limit) {
                    ...
                    html += '<a class="btn list_maped_resources tooltips transparent rl_large_icon" '
                        + data_mode + ' data-type="' + type
                        + '" data-original-title="<?php echo_html2view($text_view_more);
                            ?>"><i class="fa fa-folder-open"></i></a></div>';
                    html += '<div class="caption center ellipsis"><a href="#" class="list_maped_resources resource_edit" '
                        + data_mode + ' data-type="' + type + '"><?php echo_html2view(
                            $text_view_more
                        ); ?></a></div><br />';
                    html += '</div>';
                }
                ...
                html += '<div class="col-md-1 col-sm-2 col-xs-6 reslibrary_block">' +
                    ...
                    '<form action="<?php echo $rl_upload; ?>&type=' + type +
                    '" method="POST" enctype="multipart/form-data"><input type="file" name="files[]" multiple="" class="hide">';
                html += '<a class="btn resource_add tooltips transparent rl_large_icon" ' + data_mode +
                    ' data-type="' + type + '" data-original-title="<?php
                        echo_html2view($text_add_media); ?>"><i class="fa fa-plus-circle"></i></a>';
                html += '</form></div>';
                html += '<div class="caption center ellipsis"><a href="#" class="resource_add" ' +
                    data_mode + ' data-type="' + type + '"><?php echo_html2view($text_add_media) ?></a></div><br />';
                html += '</div>';
            ...
            success: function (item) {
                ...
                if (item != null) {
                    ...
                    html += '<a class="btn resource_add tooltips transparent" ' +
                        ...
                        'data-original-title="<?php echo_html2view(
                            $text_add_media
                        ) ?>"><img src="<?php echo $template_dir
                            .'image/icons/icon_add_media.png'; ?>" alt="<?php echo_html2view(
                            $text_add_media
                        ); ?>" width="100" /></a>';
                    html += '</div></div>';
                ...
                if (field && field.length > 0) {
                    $('#' + wrapper_id).html(html)
                        .find('.resource_delete').on('click', function () {
    ...
    var bind_rl = function (elm) {
        ...
        // was made so because user must to confirm action by another js-click-event
        ?>
            ...
            if (!URL) {
                if ($('#resource_types_tabs li.active').attr('data-type')) {
                    URL = $('div.fileupload_drag_area').find('form').attr('action')
                        + '&type=' + $('#resource_types_tabs li.active').attr('data-type');
                } else {
                    URL = $('div.fileupload_drag_area').find('form').attr('action');
                }
            }
