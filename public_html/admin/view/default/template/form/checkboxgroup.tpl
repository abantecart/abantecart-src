...
?><label for="<?php echo $check_id ?>"><div class="afield acheckbox <?php echo (in_array($v, $value) ? 'checked':'') ?>"><span>
                <input id="<?php echo $check_id ?>" type="checkbox" value="<?php echo $v ?>" <?php echo ($style ? 'class="'.$style.'"':''); ?> name="<?php echo $name ?>" <?php echo (in_array($v, $value) ? ' checked="checked" ':'') ?> ovalue="<?php echo (in_array($v, $value) ? 'true':'false') ?>" style="opacity: 0;" />
