...
var mcei = {
    ...
    autop: true,
    indent: false,
    toolbar: "undo redo bold italic strikethrough bullist forecolor backcolor numlist blockquote hr alignleft aligncenter alignright link spellchecker dfw fullscreen table",
    selector: 'textarea',
    valid_elements: '*[*]',
...
var wrapConfirmDelete = function(){
    ...
    $('a[data-confirmation="delete"]').each( function(){
        ...
        popover = '<div class="confirm_popover dropdown-menu dropdown-menu-right alert alert-danger" role="menu">'+
                    ...
                    '<div class="center">'+
                    '<a class="btn btn-danger" ' + action + ' ><i class="fa fa-trash-o"></i>&nbsp;' +
                    <?php js_echo($text_yes);?> +
                    '</a>&nbsp;&nbsp;' +
                    '<a class="btn btn-default"><i class="fa fa-undo"></i>&nbsp;'+
                    <?php js_echo($text_no);?> +
                    '</a></div></div>';
        $(this).after(popover);
