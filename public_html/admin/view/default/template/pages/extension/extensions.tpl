...
<div id="content" class="panel panel-default">
    <div class="panel-heading col-xs-12">
        <div class="primary_content_actions pull-left">
            ...
            if (!$mp_connected) { ?>
                <div class="btn-group">
                    <a class="btn btn-orange mp-connect tooltips" title="<?php
                    echo $text_marketplace_connect; ?>" data-toggle="modal" data-target="#amp_modal">
                        <i class="fa fa-sign-in fa-fw"></i> <?php
                        echo $text_connect ?>
                    </a>
                </div>
            <?php
            ...
            <div class="btn-group">
                ...
                echo $btn_extensions_store->href; ?>">
                    <i class="fa fa-cloud-download fa-fw"></i> <?php
                    echo $btn_extensions_store->text ?>
                </a>
            </div>
            ...
            if ($setting_url) { ?>
                <div class="btn-group">
                    <a class="btn btn-default tooltips" href="<?php
                    echo $setting_url; ?>" title="<?php
                    echo $text_configuration_settings; ?>">
    ...
    <div class="panel-footer col-xs-12">
        <div class="text-center">
            <a href="<?php
            echo $more_extensions_url; ?>" class="btn btn-orange lock-on-click">
                <i class="fa fa-puzzle-piece fa-fw"></i> <?php
                echo $text_more_extensions; ?>
            </a>
        </div>
    </div>
...
echo $this->html->buildElement(
    [
        ...
        'title'      => $text_license,
        'content'    => '',
        'footer'     => '<button type="button" class="btn btn-default" data-dismiss="modal" id="license_cancel">'
...
<script type="application/javascript">
    function appendMissingRowsFromPayload($grid, userdata, data) {
        if (!data || !data.rows || !data.rows.length) {
            return;
        }

        const existingRowIds = {};
        $grid.find('tbody tr[id]').each(function () {
            existingRowIds[this.id] = true;
        });

        const colModel = $grid.jqGrid('getGridParam', 'colModel') || [];
        for (let r = 0; r < data.rows.length; r++) {
            const payloadRow = data.rows[r];
            if (!payloadRow || existingRowIds[payloadRow.id]) {
                continue;
            }

            const rowData = {};
            for (let c = 0; c < colModel.length; c++) {
                rowData[colModel[c].name] = payloadRow.cell[c];
            }

            $grid.jqGrid('addRowData', payloadRow.id, rowData, 'last');
            const rowClasses = userdata && userdata.classes ? userdata.classes[payloadRow.id] : '';
            if (rowClasses) {
                $('#' + payloadRow.id).addClass(rowClasses);
            }
            existingRowIds[payloadRow.id] = 1;
        }
    }

    function hasPrependedPopular(userdata) {
        if (!userdata || !userdata.popular_prepend) {
            return false;
        }

        for (const rowId in userdata.popular_prepend) {
            if (!Object.prototype.hasOwnProperty.call(userdata.popular_prepend, rowId)) {
                continue;
            }
            if (userdata.popular_prepend[rowId]) {
                return true;
            }
        }
        return false;
    ...
    function hideRegularDuplicatesForPrependedPopular($grid, userdata) {
        if (!userdata || !userdata.extension_id || !userdata.popular_prepend) {
            return;
        }

        const extensionIdByRowId = userdata.extension_id;
        const prependedByRowId = userdata.popular_prepend;
        const prependedPopularExt = {};
        let hasPrependedPopular = false;

        $grid.find('tbody tr[id]').each(function () {
            const rowId = this.id;
            if (prependedByRowId[rowId]) {
                const prependedExt = extensionIdByRowId[rowId];
                if (prependedExt) {
                    prependedPopularExt[prependedExt] = 1;
                    hasPrependedPopular = true;
                }
            }
        });

        if (!hasPrependedPopular) {
            return;
        }
        ...
        $grid.find('tbody tr[id]').each(function () {
            ...
            const extensionId = extensionIdByRowId[rowId];
            if (extensionId && prependedPopularExt[extensionId]) {
                $(this).hide();
            }
        });
    }

    function syncVisibleRowsPagerInfo($grid) {
        const records = parseInt($grid.jqGrid('getGridParam', 'records'), 10) || 0;
        const rowNum = parseInt($grid.jqGrid('getGridParam', 'rowNum'), 10) || 0;
        const page = parseInt($grid.jqGrid('getGridParam', 'page'), 10) || 1;
        const visibleRows = $grid.find('tbody tr[id]:visible').length;

        let from = 0;
        let to = 0;
        if (records > 0 && rowNum > 0) {
            from = (page - 1) * rowNum + 1;
            to = Math.min(from + Math.max(visibleRows, 0) - 1, records);
            if (to < from) {
                to = from;
            }
        }

        const recordText = ($.jgrid && $.jgrid.defaults && $.jgrid.defaults.recordtext)
            ? $.jgrid.defaults.recordtext
            : 'View {0} - {1} of {2}';
        const text = recordText
            .replace('{0}', from)
            .replace('{1}', to)
            .replace('{2}', records);
        $('#extension_grid_pager .ui-paging-info').text(text);
    }

    /* run after grid load */
    var extension_grid_ready = function (data) {

        const $grid = $('#extension_grid');
        const userdata = data.userdata;
        const hasPrepended = hasPrependedPopular(userdata);
        if (hasPrepended) {
            appendMissingRowsFromPayload($grid, userdata, data);
            hideRegularDuplicatesForPrependedPopular($grid, userdata);
            syncVisibleRowsPagerInfo($grid);
        }
        ...
        $('.grid_action_expired').each(function () {
            if (userdata.hasOwnProperty('mp_product_url')) {
                var row_id = $(this).parents('tr').attr('id');
                $(this).attr('target', '_new')
                    .attr('href', userdata.mp_product_url[row_id]);
            }
        });
        //check dependencies before disabling of extension
        $('td[aria-describedby="extension_grid_status"] button').click(function () {
            ...
            if (value === 1 && switcher.attr('data-orgvalue') !== 0) {
                ...
                var modalElm = $('#license_modal');
                modalElm.modal({
                    show: false,
                    remote: '<?php echo $dependants_url; ?>&extension=' + extension
                });
